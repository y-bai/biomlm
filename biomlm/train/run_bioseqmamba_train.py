...
def main():
    ...
    logger.info(f"Max length: {tokenization_config.model_max_length}")

    logger.info(f">>>loading tokenized dataset for {dataset_config.dataset_name}")
    ...
    clm_tokenized_ds = load_from_disk(tokenization_config.tokenized_dataset_dir)
    ...
    if training_config.do_eval:
        if "test" not in clm_tokenized_ds:
